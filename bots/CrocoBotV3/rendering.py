...
def render_loop(bot):
    ...
    while running:
        ...
        with bot.lock:
            enemies = bot.enemies.copy()
            danger_vector = bot.danger_vector
            enemy_projection = bot.enemy_projection
        radar_dir = bot.radar_direction
        ...
        for idx, d in enemies.items():
            ...
            if idx in enemy_projection:
                x, y = to_pg_pos(enemy_projection[idx], arena_height)
                pygame.draw.circle(screen, (255, 180, 0), (x, y), 18, 1)
